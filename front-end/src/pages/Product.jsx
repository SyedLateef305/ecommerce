      ...
      <div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>
            ...
            <div className='flex flex-col gap-4 my-8'>
                ...
                <div className='flex gap-2'>
                  {productData.sizes.map((item,index)=>(
                    <button onClick={() => setSize(item)} className={`border py-2 px-4 cursor-pointer transition ${item === size ? 'border-orange-500 bg-orange-100': 'bg-gray-100 hover:bg-gray-200'}`} key={index}>{item}</button>
                  ))}
                  </div>
              </div>
        ...
        <div className='mt-20'>
          ...
          <div className='flex flex-col gap-4 border px-6 py-6 text-sm text-gray-500'>
            ...
            <p>E-commerce websites typoically display products or services along with detailed descriptions,images,prices, and any available variations(e.g., sizes,colors). Each product usually has its own 
              dedicated page with related information
            </p>
          </div>
        </div>
